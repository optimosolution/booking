...
class User extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('registerDate, lastvisitDate, address, company, shop_id', 'safe'),
            ...
            array('id, name, , shop_id, username, email, password, registerDate, lastvisitDate, activation, group_id, country, state, city, phone, mobile, fax, website, user_type, status, storeowner', 'safe', 'on' => 'search'),
        );
    }
    ...
    public static function get_profile_picture($id) {
        ...
        $filePath = Yii::app()->basePath . '/../uploads/profile_picture/' . $value->profile_picture;
        if ((is_file($filePath)) && (file_exists($filePath))) {
            return CHtml::image(Yii::app()->baseUrl . '/uploads/profile_picture/' . $value->profile_picture, 'Profile Picture', array('alt' => $value->name, 'class' => 'thumblin', 'title' => $value->name, 'style' => 'width:120px;'));
        } else {
            return CHtml::image(Yii::app()->baseUrl . '/uploads/profile_picture/profile.jpg', 'Profile Picture', array('alt' => $value->name, 'class' => 'thumblin', 'title' => $value->name, 'style' => 'width:120px;'));
        }
    }
    ...
    public static function get_profile_picture90px($id) {
        ...
        $filePath = Yii::app()->basePath . '/../uploads/profile_picture/' . $value->profile_picture;
        if ((is_file($filePath)) && (file_exists($filePath))) {
            return CHtml::image(Yii::app()->baseUrl . '/uploads/profile_picture/' . $value->profile_picture, 'Profile Picture', array('alt' => $value->name, 'class' => 'thumblin', 'title' => $value->name, 'style' => 'width:90px;'));
        } else {
            return CHtml::image(Yii::app()->baseUrl . '/uploads/profile_picture/profile.jpg', 'Profile Picture', array('alt' => $value->name, 'class' => 'thumblin', 'title' => $value->name, 'style' => 'width:90px;'));
        }
    }
    /**
     * Send mail method
