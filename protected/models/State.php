...
class State extends CActiveRecord {
    ...
    public static function get_sate_new($controller, $field) {
    ...
    public static function get_sate_edit($controller, $field, $id) {
        $rValue = Yii::app()->db->createCommand()
                ->select('id,country_id,state_name')
                ->from('{{state}}')
                ->where('published=1')
                ->order('state_name')
                ->queryAll();
        echo '<select id="' . $controller . '_' . $field . '" name="' . $controller . '[' . $field . ']" class="span5 input-large">';
        echo '<option value="">--select state--</option>';
        foreach ($rValue as $key => $values) {
            if ($values["id"] == $id) {
