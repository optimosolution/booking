...
class Currency extends CActiveRecord {
    ...
    public static function get_currency_short_code($id) {
        $value = Currency::model()->findByAttributes(array('id' => $id));
        if (empty($value->currency_code_3)) {
            return null;
        } else {
            return $value->currency_code_3;
        }
    }
    public static function get_currency_symbol($id) {
        $value = Currency::model()->findByAttributes(array('id' => $id));
